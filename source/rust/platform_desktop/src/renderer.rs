...
impl Renderer {
	...
	fn draw_craft(&mut self, body: &Body, camera: &Camera, icon_id: &str) {
		...
		let mut transform = self.perspective()
			...
			.mul(&Matrix4::from_translation(&body.position.to_vector_f32()));
		...
		self.draw_text(
			...
			.as_slice(),
			&transform.mul(&Matrix4::from_translation(&text_position.to_vector_f32())),
		);
	...
	fn draw_ui_overlay(&mut self, attitude: Radians) {
		...
		self.draw_text(
			...
			&projection.mul(&Matrix4::from_translation(&Vec2(left + 20.0, bottom + 60.0).to_vector_f32()))
		);
		self.draw_text(
			"Change course with the left and right cursor keys",
			&projection.mul(&Matrix4::from_translation(&Vec2(left + 20.0, bottom + 40.0).to_vector_f32())),
		);
		self.draw_text(
			"Shoot missiles with Enter",
			&projection.mul(&Matrix4::from_translation(&Vec2(left + 20.0, bottom + 20.0).to_vector_f32())),
		);

		self.draw_text(
			format!("{:+04i}", attitude.degrees()).as_slice(),
			&projection.mul(&Matrix4::from_translation(&Vec2(right - 50.0, bottom + 40.0).to_vector_f32())),
		);
	}
	...
	fn draw_text(&mut self, text: &str, transform: &Transform) {
		...
		for c in text.chars() {
			...
			if c != ' ' {
				...
				self.draw_icon(
					&icon,
					&transform.mul(&Matrix4::from_translation(&(offset + total_advance).to_vector_f32())),
				);
			}
	...
	fn draw_icon(&mut self, icon: &Icon, transform: &Transform) {
		let params = IconParams {
			transform: transform.mul(&Matrix4::from_translation(&icon.offset.to_vector_f32())).into_fixed(),
			tex      : icon.param,
		};
